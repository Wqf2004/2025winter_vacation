...
#define CLOSE_TIME 500 // 定义银行关门时间:也就是银行一天营业8个半小时左右
#define MAX_QUEUE 4   // 定义最大队列数
/*定义银行客户*/
...
double simulate()
{
    ...
    while((&eventlist)->head != NULL)
    {
        ...
        if(evItem.nType == -1)
        {
            // 此刻有顾客到达了，就让他进入统一队列，此外还可以生成下一位顾客的到达时刻
            clientNum++; // 客户数加1
            /*生成新客户并输入随机数*/
            Client newClient;
            newClient.num = clientNum;
            newClient.arrivalTime = overTime;
            newClient.duration = randInterval(5,35); // 生成业务处理时间
            // 此刻，查找是否有空闲窗口
            int freeWindowIndex = -1;
            for (int i = 0; i < MAX_QUEUE; i++) {
                if (isEmpty(&queue[i])) {
                    freeWindowIndex = i;
                    break;
            ...
            // 如果有，就直接让新到达的顾客进入到窗口接受服务;否则进入统一长队中等待
            if (freeWindowIndex != -1) {
                printf("客户%d于银行开始营业的%d分钟来到银行，直接进入%d号窗口\n", newClient.num, newClient.arrivalTime, freeWindowIndex+1);
                enqueue(&queue[freeWindowIndex], newClient);
                //添加离开事件
                insertEvent(&eventlist, overTime + newClient.duration, freeWindowIndex);
                newClient.waitTime = 0;
            }
            else
            {
                printf("客户%d于银行开始营业的%d分钟来到银行，并排入统一长队\n", newClient.num, newClient.arrivalTime);
                enqueue(&mainQueue, newClient); // 新客户排入统一长队
            }
            ...
            int win = evItem.nType; // 获得离开客户所在的窗口号
            Client outClient = queue[win].front->data; // 获取头结点数据
            // win窗口在当前客户离开后一定为空，于是从统一长队中取出队首入队
            if(dequeue(&queue[win])){ // 从队列中删除一个客户
                ...
                int waitInLine = evItem.occurTime - outClient.arrivalTime; // 排队时间
                if(!isEmpty(&mainQueue)) // 如果统一长队不为空，就将统一长队中的第一个客户进入该窗口
                {
                    enqueue(&queue[win], mainQueue.front->data);
                    // 将该事件加入事件表中
                    insertEvent(&eventlist, evItem.occurTime + mainQueue.front->data.duration, win);
                }   
